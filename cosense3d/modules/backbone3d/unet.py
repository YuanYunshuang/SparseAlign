...
class Unet(BaseModule):
    ...
    def __init__(self,
        ...
        self.d = d
        self.expand_2d = expand_2d
        self.expand_3d = expand_3d
    ...
    def _init_unet_layers(self, kernel_size_layer1=3):
        ...
        kernel = [3, 3, 3]
        strides = [2, 1, 1]
        expand = [self.expand_3d, False, False]
        self.conv1 = get_conv_block([self.enc_dim, self.enc_dim*2, self.enc_dim*2],
                                    kernel, s=[[1, 1, zs1], 1, 1], **kwargs)
        self.conv2 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*2],
                                    kernel, s=strides, **kwargs)
        self.conv3 = get_conv_block([self.enc_dim*2, self.enc_dim*4, self.enc_dim*2],
                                    kernel, s=strides, expand=expand, **kwargs)
        self.conv4 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*4],
                                    kernel, s=strides, expand=expand, **kwargs)

        if self.max_resolution <= 4:
            self.trconv4 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, expand=expand, **kwargs)
        if self.max_resolution <= 2:
            self.trconv3 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, expand=expand, **kwargs)
        if self.max_resolution <= 1:
            self.trconv2 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, **kwargs)
        self.compr_h = minkconv_conv_block(self.enc_dim*4, self.enc_dim*4, kernel, [1, 1, 3], **kwargs)
        self.out_layer = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.out_dim], [3, 3, 1], s=[1, 1, 1],
                                        expand=[self.expand_2d, self.expand_2d, False], **kwargs)
    ...
    def forward(self, points: list, **kwargs):
        ...
        for k in self.cache_strides:
            ...
            out_tensor = self.compr_h(vars[f'p{k}_cat'])
            out_tensor = self.out_layer(out_tensor)
...
class MinkUnet(BaseModule):
    ...
    def _init_unet_layers(self, kernel_size_layer1=3):
        ...
        kernel = [3, 3, 3]
        strides = [2, 1, 1]
        self.conv1 = get_conv_block([self.enc_dim, self.enc_dim*2, self.enc_dim*2], kernel, s=[1, 1, 1], **kwargs)
        self.conv2 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*2], kernel, s=strides, **kwargs)
        self.conv3 = get_conv_block([self.enc_dim*2, self.enc_dim*4, self.enc_dim*2], kernel, s=strides, **kwargs)
        self.conv4 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*4], kernel, s=strides, **kwargs)

        if self.max_resolution <= 4:
            self.trconv4 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2], kernel, s=strides, tr=True, **kwargs)
        if self.max_resolution <= 2:
            self.trconv3 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2], kernel, s=strides, tr=True, **kwargs)
        if self.max_resolution <= 1:
            self.trconv2 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2], kernel, s=strides, tr=True, **kwargs)
        self.compr_h = minkconv_conv_block(self.enc_dim*4, self.enc_dim*4, kernel, [1, 1, 5], **kwargs)
        self.out_layer = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.out_dim], [3, 3, 1], s=[1, 1, 1],
                                        expand=[True, True, True], **kwargs)
...
class UnetDilConv(BaseModule):
    ...
    def __init__(self,
                 ...
                 expand_3d=True,
                 **kwargs):
        super(UnetDilConv, self).__init__(**kwargs)
        update_me_essentials(self, data_info)
        self.z_max = int((self.lidar_range[5] - self.lidar_range[2]) / self.voxel_size[2])
        self.stride = stride
        self.in_dim = in_dim
    ...
    def _init_unet_layers(self, kernel_size_layer1=3):
        ...
        dilation = [2, 1, 1]
        self.conv1 = get_conv_block([self.enc_dim, self.enc_dim*2, self.enc_dim*2],
                                    kernel, s=[[1, 1, zs1], 1, 1], dilation=dilation, **kwargs)
        self.conv2 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*2],
                                    kernel, s=strides, dilation=dilation, **kwargs)
        self.conv3 = get_conv_block([self.enc_dim*2, self.enc_dim*4, self.enc_dim*2],
                                    kernel, s=strides, dilation=dilation, expand=expand, **kwargs)
        self.conv4 = get_conv_block([self.enc_dim*2, self.enc_dim*2, self.enc_dim*4],
                                    kernel, s=strides, dilation=dilation, expand=expand, **kwargs)

        if self.max_resolution <= 4:
            self.trconv4 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, dilation=dilation, expand=expand, **kwargs)
        if self.max_resolution <= 2:
            self.trconv3 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, dilation=dilation, expand=expand, **kwargs)
        if self.max_resolution <= 1:
            self.trconv2 = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.enc_dim*2],
                                          kernel, s=strides, tr=True, dilation=dilation, **kwargs)
        self.compr_h = minkconv_conv_block(self.enc_dim*4, self.enc_dim*4, kernel, [1, 1, 3], **kwargs)
        self.out_layer = get_conv_block([self.enc_dim*4, self.enc_dim*2, self.out_dim], [3, 3, 1], s=[1, 1, 1],
                                        expand=[self.expand_2d, self.expand_2d, False], **kwargs)
