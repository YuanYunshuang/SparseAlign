...
class BEVSemsegCAV(BaseCAV):
    ...
    def forward_local(self, tasks, training_mode, **kwargs):
        ...
        # tasks[grad_mode].append((self.id, '13:semseg_head_local', {}))
    ...
    def forward_head(self, tasks, training_mode, **kwargs):
        if self.is_ego:
        #     tasks['with_grad'].append((self.id, '22:det_head', {}))
            tasks['with_grad'].append((self.id, '23:semseg_head', {}))
        return tasks

    def loss(self, tasks, **kwargs):
        if self.is_ego:
            # tasks['loss'].append((self.id, '31:semseg_head_local', {}))
            # tasks['loss'].append((self.id, '32:det_head', {}))
            tasks['loss'].append((self.id, '33:semseg_head', {}))
        return tasks
