...
from cosense3d.utils.pclib import tf2pose, pose2tf
...
class GenerateErrorPose:
    ...
    def __call__(self, data_dict):
        err_poses = []
        for i in range(len(data_dict['valid_agent_ids'])):
            err = data_dict['loc_err'][i]
            gt_pose = data_dict['lidar_poses'][i]
            err_pose = tf2pose(gt_pose)
            err_pose[0] += err[0]
            err_pose[1] += err[1]
            err_pose[5] += err[2]
            err_pose = pose2tf(err_pose)
            err_poses.append(err_pose)
        data_dict['err_poses'] = err_poses
        return data_dict